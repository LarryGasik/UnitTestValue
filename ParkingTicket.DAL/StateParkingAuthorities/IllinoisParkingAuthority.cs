...
public class IllinoisParkingAuthority : IStateParkingAuthority
{
    public List<ParkingTicketDto> GetTicketsFromTag(string tag)
    {
        ...
        if (tag == "Larry")
            ...
            {
                DateOfOffense = DateTime.MinValue, Fine = 400, Offense = "Restricted Parking Zone", State = "IL",
                TicketID = Guid.Empty
            });
        if (tag == "Diesel")
            tickets.Add(new ParkingTicketDto
            {
                DateOfOffense = DateTime.MinValue, Fine = 100, Offense = "Fire Hydrant", State = "IL",
                TicketID = Guid.Empty
            });
        return tickets;
    }
